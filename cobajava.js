...
function getIconSizeFromZoom(zoom) {
...
function updateVesselsOnMap(vesselList) {
    vesselList.forEach(function(vessel) {
        if (vessel.lat && vessel.lon) { // Pastikan koordinat valid
            ...
            const iconSize = getIconSizeFromZoom(map.getZoom());

            // Pilih ikon berdasarkan kombinasi owner dan shipType
            let shipIconUrl;
            if (vessel.owner === "Ship") {
                switch (vessel['ship type']) {
                    case "Cargo Ship": shipIconUrl = 'cargo.svg'; break;
                    case "Tanker": shipIconUrl = 'tanker.svg'; break;
                    case "Passenger Ship": shipIconUrl = 'passenger.svg'; break;
                    case "Fishing Vessel": shipIconUrl = 'fishing.svg'; break;
                    default: shipIconUrl = 'sna.svg'; break;
                }
...
map.on("zoomend", () => {
    ...
    const newSize = getIconSizeFromZoom(zoom);
